...
function showUser(user){currentUserId=user?.id||null;if(!user){show('loginSection');return}if(user.mustChangePassword){showPassword(true);return}if(requestedNext){location.replace(nextPage);return}$('#signedInName').textContent=`你好，${user.displayName||user.username}`;show('signedInSection')}
...
if(new URLSearchParams(location.search).get('notice')==='local-draft-cleanup')message($('#loginMessage'),'已退出登录，但本机恢复稿未能清除；共用设备请清除此网站的浏览器数据。',true);
