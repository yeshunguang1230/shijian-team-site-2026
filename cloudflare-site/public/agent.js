...
function matchSources(query){
  const parts=String(query).toLowerCase().match(/[\u4e00-\u9fff]+|[a-z0-9]+/g)||[];
  const terms=[...new Set(parts.flatMap(part=>/^[\u4e00-\u9fff]+$/.test(part)&&part.length>2?[part,...Array.from({length:part.length-1},(_,i)=>part.slice(i,i+2))]:[part]))].filter(x=>x.length>1);
  return state.kb.map(source=>({...source,score:terms.reduce((n,term)=>n+(source.title.toLowerCase().includes(term)?4:0)+(source.content.toLowerCase().includes(term)?1:0),0)})).filter(x=>x.score>0).sort((a,b)=>b.score-a.score).slice(0,5);
}
function renderEvidence(sources){
  $('#evidenceList').innerHTML=sources.length?sources.map(s=>`<article class="evidence"><strong>[${esc(s.id)}] ${esc(s.title)}</strong><small>${esc(s.author)} · ${esc(s.date)} · ${esc(s.reliability)}</small><small>出处：${esc(s.locator||'演示材料 / 出处待补充')}</small><p>${esc(s.content.slice(0,180))}${s.content.length>180?'…':''}</p></article>`).join(''):'<div class="muted">没有命中资料，请缩小问题范围或等待团队补充资料。</div>';
}
...
function excerpts(sources){return sources.map(s=>({id:s.id,title:s.title,author:s.author,locator:s.locator,reliability:s.reliability,content:s.content.slice(0,2500)}))}
async function analyze(){
  ...
  if(canUseAI()&&sources.length){try{data=await callAI('只使用给定材料，资料不足时明确说明。返回 JSON：answer、claims（type、text、source_ids）、timeline、people、uncertainty。事实必须有有效引用。问题：'+question.slice(0,2000)+'\nSOURCES：'+JSON.stringify(excerpts(sources)));demo=false}catch(error){failure=error.message}}
...
function showQuiz(){
  const quiz=state.quiz;$('#quizQuestion').textContent=quiz.question;$('#quizSource').textContent=`${quiz.demo?'规则示例题 · 非 AI 生成':'AI 生成题 · 需核验'}\n材料 [${quiz.source.id}] ${quiz.source.title}：${quiz.source.content.slice(0,8000)}`;$('#quizSource').classList.remove('hidden');$('#studentAnswer').value='';$('#gradeResult').className='result empty';$('#gradeResult').textContent='提交后显示反馈，规则演示不判断史实真假。';
...
function newQuiz(){
  ...
  const source=state.kb[index%state.kb.length];
  state.quiz={source,demo:true,question:`请结合 [${source.id}] 的材料，提出一个明确观点，引用材料中的证据，并解释它为什么支持观点。`};showQuiz();
}
async function generateQuiz(){
  newQuiz();if(!canUseAI())return;
  try{const sources=state.kb.slice(0,5);const data=await callAI('只根据给定材料生成一道可作答的材料分析题。返回 JSON：question、source_id、focus。source_id 必须取自材料。SOURCES：'+JSON.stringify(excerpts(sources)));const source=sources.find(x=>x.id===data.source_id);if(!source||typeof data.question!=='string'||!data.question.trim())throw Error('生成题目的来源无法对应，保留规则示例题。');state.quiz={source,demo:false,question:data.question};showQuiz();log('已生成材料题，来源编号已匹配，题目仍需核验。')}catch(error){log('出题未完成：'+error.message+' 当前为规则示例题。')}
}
